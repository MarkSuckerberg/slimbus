...
class PlayerController extends Controller
{
  ...
  {
    $player = $this->getPlayerByCkey(htmlspecialchars($args['ckey']), TRUE);
    if (!$player->ckey) {
      return $this->view->render($this->response, 'base/error.tpl', [
        'message' => "Ckey not found",
        'code' => 404,
  ...
  {
    if ($public) {
      return $this->DB->rowObj("SELECT tbl_player.ckey
        FROM tbl_player
  ...
  public function getPlayerRoleTime($request, $response, $args)
  {
    if (isset($args['ckey'])) {
      ...
      $ckey = $this->container->get('user')->ckey;
    }
    $player = $this->getPlayerByCkey($ckey);
    if (!$player->ckey) {
      return $this->view->render($this->response, 'base/error.tpl', [
    ...
    $player->role_time = $this->getRoleData($player->ckey, $start, $end);
    $player = $this->playerModel->parsePlayer($player);
    #return $response->withJson($player->role_time);
  ...
  public function getRoleData($ckey, $start = null, $end = null)
  {
